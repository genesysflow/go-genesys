...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.userExistsByEmailExcludingStmt, err = db.PrepareContext(ctx, userExistsByEmailExcluding); err != nil {
		return nil, fmt.Errorf("error preparing query UserExistsByEmailExcluding: %w", err)
	}
	return &q, nil
}
...
func (q *Queries) Close() error {
	...
	if q.userExistsByEmailExcludingStmt != nil {
		if cerr := q.userExistsByEmailExcludingStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing userExistsByEmailExcludingStmt: %w", cerr)
		}
	}
	return err
}
...
type Queries struct {
	...
	userExistsByEmailExcludingStmt *sql.Stmt
}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		userExistsByEmailExcludingStmt: q.userExistsByEmailExcludingStmt,
	}
}
